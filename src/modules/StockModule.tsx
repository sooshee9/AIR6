// StockModule — ModuleFooter and anchor icon removed; both now live in App.tsx
...
function CalligraphicHeader({synced}:{synced:boolean}) {
  return (
    <div style={{background:'linear-gradient(135deg,#1a1200 0%,#2d1f00 40%,#1a1200 100%)',borderRadius:16,padding:'28px 36px',marginBottom:28,position:'relative',overflow:'hidden',boxShadow:'0 8px 32px rgba(0,0,0,0.35),inset 0 1px 0 rgba(212,175,55,0.3)'}}>
      <div style={{position:'absolute',top:10,left:12,fontSize:22,color:'rgba(212,175,55,0.5)',fontFamily:'serif',lineHeight:1}}>✦</div>
      <div style={{position:'absolute',top:10,right:12,fontSize:22,color:'rgba(212,175,55,0.5)',fontFamily:'serif',lineHeight:1}}>✦</div>
      <div style={{position:'absolute',bottom:10,left:12,fontSize:22,color:'rgba(212,175,55,0.5)',fontFamily:'serif',lineHeight:1}}>✦</div>
      <div style={{position:'absolute',bottom:10,right:12,fontSize:22,color:'rgba(212,175,55,0.5)',fontFamily:'serif',lineHeight:1}}>✦</div>
      ...
      <div style={{textAlign:'center',marginBottom:6}}>
        <div style={{fontFamily:'"Palatino Linotype","Book Antiqua",Palatino,"Times New Roman",serif',fontSize:38,fontWeight:400,color:'#D4AF37',letterSpacing:'0.08em',textShadow:'0 2px 12px rgba(212,175,55,0.4),0 0 40px rgba(212,175,55,0.15)',lineHeight:1.1,fontStyle:'italic'}}>
          ✦ AIRTECH ERP ✦
        </div>
        <div style={{fontFamily:'"Palatino Linotype","Book Antiqua",Palatino,"Times New Roman",serif',fontSize:15,fontWeight:400,color:'rgba(212,175,55,0.75)',letterSpacing:'0.35em',textTransform:'uppercase',marginTop:6}}>
...
const StockModule: React.FC = () => {
  ...
  // ── Vendor Issued — CORRECTED: net outstanding = raw sent − VSIR returned ──
  const vendorIssuedMap=useMemo(()=>{
    const m=new Map<string,number>();
    for(const issue of vendorIssuesState){if(!Array.isArray(issue.items))continue;for(const item of issue.items){const qty=Number(item.qty)||0;if(!qty)continue;const k=String(item.itemCode||'').trim();if(k)m.set(k,(m.get(k)||0)+qty);}}
    return m;
  },[vendorIssuesState]);
  ...
  const vsirReceivedMap=useMemo(()=>{
    const m=new Map<string,number>();
    for(const r of vsirRecordsState){const k=String(r.itemCode||'').trim();const qty=(Number(r.okQty)||0)+(Number(r.reworkQty)||0)+(Number(r.rejectQty)||0);if(k&&qty)m.set(k,(m.get(k)||0)+qty);}
    return m;
  },[vsirRecordsState]);
  const getVSIRReceivedQtyTotal=useCallback((c:string)=>vsirReceivedMap.get(String(c).trim())||0,[vsirReceivedMap]);

  // CORRECTED: matches first (correct) code — vendorIssued − vsirReceived
  const getAdjustedVendorIssuedQty=useCallback((c:string)=>Math.max(0,getVendorIssuedQtyTotal(c)-getVSIRReceivedQtyTotal(c)),[getVendorIssuedQtyTotal,getVSIRReceivedQtyTotal]);

  const vendorDeptMap=useMemo(()=>{
    const qty=new Map<string,number>(),ok=new Map<string,number>();
    for(const order of vendorDeptState){if(!Array.isArray(order.items))continue;for(const item of order.items){const k=String(item.itemCode||'').trim();if(typeof item.qty==='number')qty.set(k,(qty.get(k)||0)+item.qty);if(typeof item.okQty==='number')ok.set(k,(ok.get(k)||0)+item.okQty);}}
    return{qty,ok};
  },[vendorDeptState]);

  const purchaseQtyMap=useMemo(()=>{
    const m=new Map<string,number>();
    for(const entry of purchaseOrdersState){if(Array.isArray(entry.items)){for(const item of entry.items){if(typeof item.qty==='number'){const k=String(item.itemCode||'').trim();m.set(k,(m.get(k)||0)+item.qty);}}}else if(entry.itemCode&&typeof entry.qty==='number'){const k=String(entry.itemCode).trim();m.set(k,(m.get(k)||0)+entry.qty);}}
    return m;
  },[purchaseOrdersState]);

  const indentQtyMap=useMemo(()=>{
    const m=new Map<string,number>();
    for(const indent of indentState){if(!Array.isArray(indent.items))continue;for(const item of indent.items){if(item.itemCode&&typeof item.qty==='number'){const k=String(item.itemCode).trim();m.set(k,(m.get(k)||0)+item.qty);}}}
    return m;
  },[indentState]);
  ...
  const psirOkMap=useMemo(()=>{
    ...
    const proc=(item:any)=>{const name=normalize(item.itemName||item.Item||'');const code=normalize(item.itemCode||item.Code||item.CodeNo||'');const ok=(item.okQty!=null&&Number(item.okQty)>0)?Number(item.okQty):(Number(item.qtyReceived)||0);add(name,code,ok);};
  ...
  const inHouseMap=useMemo(()=>{
    const m=new Map<string,number>();
    for(const issue of inHouseIssuesState){if(!Array.isArray(issue.items))continue;for(const item of issue.items){const code=normalize(item.itemCode||'');const name=normalize(item.itemName||'');const txType=item.transactionType||'';const qty=Number(item.issueQty||item.qty||0);if(!qty)continue;m.set(`code:${code}:type:${txType}`,(m.get(`code:${code}:type:${txType}`)||0)+qty);m.set(`code:${code}:type:*`,(m.get(`code:${code}:type:*`)||0)+qty);m.set(`name:${name}`,(m.get(`name:${name}`)||0)+qty);m.set(`name:${name}:code:${code}`,(m.get(`name:${name}:code:${code}`)||0)+qty);if(txType==='Stock')m.set(`stock:name:${name}:code:${code}`,(m.get(`stock:name:${name}:code:${code}`)||0)+qty);}}
    return m;
  ...
  const getPSIROkQtyTotal=useCallback((itemName:string,itemCode?:string)=>Math.max(psirOkMap.get('name:'+normalize(itemName))||0,psirOkMap.get('code:'+normalize(itemCode))||0),[psirOkMap,normalize]);
  const getInHouseQtyByTxType=useCallback((c:string,tx:string)=>inHouseMap.get(tx==='*'?`code:${normalize(c)}:type:*`:`code:${normalize(c)}:type:${tx}`)||0,[inHouseMap,normalize]);
  const getInHouseIssuedQtyByItemName=useCallback((itemName:string,itemCode?:string)=>Math.max(inHouseMap.get(`name:${normalize(itemName)}`)||0,inHouseMap.get(`code:${normalize(itemCode)}:type:*`)||0),[inHouseMap,normalize]);
  const getInHouseStockOnly=useCallback((itemName:string,itemCode?:string)=>inHouseMap.get(`stock:name:${normalize(itemName)}:code:${normalize(itemCode)}`)||0,[inHouseMap,normalize]);

  const getVendorQty=useCallback((c:string)=>Math.max(0,(vendorDeptMap.qty.get(String(c).trim())||0)-getVendorIssuedQtyTotal(c)),[vendorDeptMap,getVendorIssuedQtyTotal]);
  const getVendorOkQty=useCallback((c:string)=>Math.max(0,getVendorDeptOkQtyTotal(c)-getInHouseQtyByTxType(c,'Vendor')),[getVendorDeptOkQtyTotal,getInHouseQtyByTxType]);
  const getPurStoreOkQty=useCallback((itemName:string,itemCode?:string)=>Math.max(0,getPSIROkQtyTotal(itemName,itemCode)-getInHouseQtyByTxType(itemCode||'','Purchase')-getVendorIssuedQtyTotal(itemCode||'')),[getPSIROkQtyTotal,getInHouseQtyByTxType,getVendorIssuedQtyTotal]);
  const computeClosingStock=useCallback((itemName:string,itemCode:string,stockQty:number)=>(Number(stockQty)||0)+getPurStoreOkQty(itemName,itemCode)+getVendorOkQty(itemCode)-getInHouseStockOnly(itemName,itemCode),[getPurStoreOkQty,getVendorOkQty,getInHouseStockOnly]);

  const liveCalc=useMemo(()=>{
    const{itemName,itemCode,stockQty}=form;
    if(!itemName&&!itemCode)return null;
    return{
      indentQty:getIndentQtyTotal(itemCode),purchaseQty:getPurchaseQtyTotal(itemCode),
      vendorQty:getVendorQty(itemCode),purStoreOkQty:getPurStoreOkQty(itemName,itemCode),
      vendorOkQty:getVendorOkQty(itemCode),inHouseIssuedQty:getInHouseIssuedQtyByItemName(itemName,itemCode),
      vendorIssuedQty:getAdjustedVendorIssuedQty(itemCode),
      closingStock:computeClosingStock(itemName,itemCode,stockQty),
    };
  },[form,getIndentQtyTotal,getPurchaseQtyTotal,getVendorQty,getPurStoreOkQty,getVendorOkQty,getInHouseIssuedQtyByItemName,getAdjustedVendorIssuedQty,computeClosingStock]);

  const filteredRecords=useMemo(()=>{if(!filterText)return records;const q=filterText.toLowerCase();return records.filter(r=>[r.itemName,r.itemCode,r.batchNo].some(f=>String(f||'').toLowerCase().includes(q)));},[records,filterText]);
  ...
  const handleChange=useCallback((e:React.ChangeEvent<HTMLInputElement|HTMLSelectElement>)=>{
    ...
    if(name==='itemName'){const found=itemMasterState.find(item=>item.itemCode===value);setForm(prev=>({...prev,itemName:found?found.itemName:'',itemCode:found?found.itemCode:''}));}
    else{setForm(prev=>({...prev,[name]:type==='number'?Number(value):value}));}
  },[itemMasterState]);

  const handleSubmit=useCallback(async(e:React.FormEvent)=>{
    e.preventDefault();
    if(!form.itemName){showToast('Item Name is required','error');return;}
    const autoRecord:RecordForm={...form,indentQty:getIndentQtyTotal(form.itemCode),purchaseQty:getPurchaseQtyTotal(form.itemCode),vendorQty:getVendorQty(form.itemCode),purStoreOkQty:getPurStoreOkQty(form.itemName,form.itemCode),vendorOkQty:getVendorOkQty(form.itemCode),inHouseIssuedQty:getInHouseIssuedQtyByItemName(form.itemName,form.itemCode),vendorIssuedQty:getAdjustedVendorIssuedQty(form.itemCode),closingStock:computeClosingStock(form.itemName,form.itemCode,form.stockQty)};
    if(editIdx!==null){
      const existing=records[editIdx];
      if(userUid&&typeof(existing as any).id==='string'){await updateStockRecord(userUid,String((existing as any).id),autoRecord).then(()=>{setRecords(prev=>prev.map((r,i)=>i===editIdx?{...autoRecord,id:r.id}:r));showToast('Record updated','success');}).catch(err=>showToast('Update failed: '+String((err as any)?.message||err),'error'));}
      else{setRecords(prev=>prev.map((r,i)=>i===editIdx?{...autoRecord,id:r.id}:r));showToast('Record updated (local)','success');}
      setEditIdx(null);
    }else{
      if(userUid){await addStockRecord(userUid,autoRecord).then((newId:any)=>{setRecords(prev=>[...prev,{...autoRecord,id:newId}as any]);showToast('Record added','success');}).catch(err=>{showToast('Add failed: '+String((err as any)?.message||err),'error');setRecords(prev=>[...prev,{...autoRecord,id:Date.now()}]);});}
      else{setRecords(prev=>[...prev,{...autoRecord,id:Date.now()}]);showToast('Record added (local)','info');}
    }
    setForm({...EMPTY_FORM});
  },[form,editIdx,records,userUid,getIndentQtyTotal,getPurchaseQtyTotal,getVendorQty,getPurStoreOkQty,getVendorOkQty,getInHouseIssuedQtyByItemName,getAdjustedVendorIssuedQty,computeClosingStock,showToast]);

  const handleEdit=useCallback((idx:number)=>{setForm({...records[idx]});setEditIdx(idx);window.scrollTo({top:0,behavior:'smooth'});},[records]);
  const handleDelete=useCallback(async(idx:number)=>{const rec=records[idx];if(userUid&&typeof(rec as any).id==='string'){await deleteStockRecord(userUid,String((rec as any).id)).then(()=>showToast('Record deleted','success')).catch(err=>showToast('Delete failed: '+String((err as any)?.message||err),'error'));}else{setRecords(prev=>prev.filter((_,i)=>i!==idx));showToast('Record deleted (local)','success');}},[ records,userUid,showToast]);
    ...
    <>
      ...
      <div style={{background:S.bg,fontFamily:"'Geist','DM Sans',system-ui,sans-serif"}}>
        <div style={{maxWidth:1600,margin:'0 auto',padding:'24px 24px 32px'}}>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{display:'flex',gap:14,flexWrap:'wrap',marginBottom:16}}>
                <Field label="Item Name" style={{flex:'1 1 220px'}}>
                  <select name="itemName" className="sk-input" style={{...S.input,borderColor:itemMasterState.length===0?S.warning:S.borderStrong}} value={form.itemCode} onChange={handleChange}>
                ...
                <Field label="Item Code"><input style={{...S.inputDisabled,width:130}} value={form.itemCode} readOnly placeholder="Auto-filled"/></Field>
                <Field label="Batch No"><input name="batchNo" className="sk-input" style={{...S.input,width:130}} placeholder="Optional" value={form.batchNo} onChange={handleChange}/></Field>
                <Field label="Opening Stock Qty"><input type="number" name="stockQty" className="sk-input" style={{...S.input,width:120}} placeholder="0" min={0} value={form.stockQty||''} onChange={handleChange}/></Field>
                <Field label="In-House Issued Qty"><input style={{...S.inputDisabled,width:130}} value={liveCalc?.inHouseIssuedQty??0} readOnly/></Field>
                <Field label="Vendor Issued Qty"><input style={{...S.inputDisabled,width:120}} value={liveCalc?.vendorIssuedQty??0} readOnly/></Field>
              </div>
                  ...
                  <div style={{display:'flex',gap:20,flexWrap:'wrap'}}>
                    {[
                      ...
                      <div key={f.label} style={{display:'flex',flexDirection:'column',gap:2,minWidth:100}}>
                        ...
                        <span style={{fontSize:f.closing?22:18,fontWeight:800,color:f.closing?(liveCalc.closingStock<0?S.danger:S.success):f.accent?S.accent:S.textPrimary,fontVariantNumeric:'tabular-nums'}}>{f.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div style={{display:'flex',gap:10}}>
                <button type="submit" className="sk-btn" style={S.btnSuccess}>{isEditing?'✓ Update Record':'✓ Add Record'}</button>
                {isEditing&&<button type="button" className="sk-btn sk-ghost" style={S.btnGhost} onClick={()=>{setForm({...EMPTY_FORM});setEditIdx(null);}}>Cancel</button>}
                {!isEditing&&<button type="button" className="sk-btn sk-ghost" style={S.btnGhost} onClick={()=>setForm({...EMPTY_FORM})}>Clear</button>}
              </div>
            </form>
          </div>
          ...
          <div style={S.card}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10}}>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                ...
                <span style={{fontSize:12,fontWeight:600,color:S.textSecondary,background:S.bg,padding:'2px 10px',borderRadius:20,border:`1px solid ${S.border}`}}>{filteredRecords.length} of {records.length} items</span>
              </div>
              <button className="sk-btn sk-ghost" style={{...S.btnGhost,borderColor:showFilters?S.accent:S.border,color:showFilters?S.accent:S.textSecondary}} onClick={()=>setShowFilters(f=>!f)}>Search</button>
            </div>
            ...
            {showFilters&&(
              <div style={{background:S.bg,border:`1px solid ${S.border}`,borderRadius:8,padding:'14px 16px',marginBottom:16,display:'flex',gap:12,alignItems:'flex-end'}}>
                <Field label="Search items"><input className="sk-input" style={{...S.input,minWidth:260}} placeholder="Item name, code, batch no…" value={filterText} onChange={e=>setFilterText(e.target.value)}/></Field>
                {filterText&&<button className="sk-btn sk-ghost" style={{...S.btnGhost,alignSelf:'flex-end',color:S.danger,borderColor:'#FECACA'}} onClick={()=>setFilterText('')}>✕ Clear</button>}
              </div>
            )}
              ...
              <table style={{width:'100%',borderCollapse:'collapse',tableLayout:'auto'}}>
                ...
                <tbody>
                  {filteredRecords.length===0?(
                    ...
                    const vendorIssued=getAdjustedVendorIssuedQty(rec.itemCode); // corrected
                ...
                {filteredRecords.length>0&&(
                  <tfoot>
                    <tr style={{background:S.accentLight}}>
                      <td colSpan={16} style={{...S.td,fontWeight:700,color:S.textSecondary,fontSize:12,textTransform:'uppercase',letterSpacing:'0.05em'}}>Total ({filteredRecords.length} items)</td>
          ...
          {/* ModuleFooter removed — now in App.tsx below erp-content */}
        </div>
      </div>
    </>
  );
